...
// Исключение из порога: фото рецептов (*-bento-cake.webp). recipeSrcSet()
// в HomePage.astro генерирует srcset для всех десяти без проверки файлов,
// поэтому вариант обязан существовать даже у мелкой картинки. Фото
// мак-цитруса — 82 КБ, порог его пропускал, и на мобильных и планшетах
// карточка отдавала 404 вместо фото.
// До 8 сентября файлы звались q1..q10 и правило ловило /^q\d+/.
const ALWAYS = /-bento-cake\.(webp|jpe?g|png)$/i;
