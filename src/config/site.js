...
export const SITE = {
  ...
  // === Отзывы и рейтинг ===
  // Рейтинг БОЛЬШЕ НЕ ЗАДАЁТСЯ ЗДЕСЬ. С 8 сентября 2026 у каждого отзыва
  // в src/data/reviews/<lang>.json есть поле `rating` (1–5), проставленное
  // владельцем по собранной обратной связи: 44 пятёрки и 7 четвёрок.
  // Среднее и reviewCount считаются из этих чисел — см. averageRating()
  // в HomePage.astro. Это и есть желаемое состояние: рейтинг из данных,
  // а не из константы.
  //
  // Раньше тут лежало averageRating: 4.8 как заявление владельца, потому
  // что оценок по отдельным отзывам не существовало. Теперь они есть,
  // константа удалена, чтобы не было двух источников правды.
  //
  // Если оценки однажды уберут из отзывов, звёзды и AggregateRating
  // исчезнут сами — рейтинга без собранных оценок быть не должно
  // (Structured Data guidelines Google, Omnibus Directive в ЕС).
  reviews: {
    ratingSource: 'per-review ratings from owner, 2026-09-08',
  },
