...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  if (!env.DB) {
    ...
    console.warn('[waitlist] DB binding missing — email dropped:', email.replace(/(.{2}).*(@.*)/, '$1***$2'));
    return json({ ok: true, stored: false });
  }

  const lang   = String(body.lang ?? '').slice(0, 4) || null;
  const source = String(body.source ?? '').slice(0, 64) || null;
  const cf = (request as Request & { cf?: { country?: string } }).cf ?? {};

  try {
    await env.DB.prepare(
      `INSERT INTO waitlist (email, lang, source, ip_country, ts)
       VALUES (?, ?, ?, ?, ?)
       ON CONFLICT(email) DO UPDATE SET lang = excluded.lang, source = excluded.source`,
    ).bind(email, lang, source, String(cf.country ?? '').slice(0, 2) || null, Date.now()).run();
